        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    string sql = "update Пострадавшие set Фамилия = N'" + textBox1.Text + "', Имя = N'" + textBox2.Text + "' , Отчество = N'" + textBox3.Text + "', Город = '" + comboBox1.SelectedValue + "', Адрес = N'" + textBox4.Text + "', Дата_рождения = N'" + maskedTextBox1.Text + "', Номер_телефона = N'" + maskedTextBox3.Text + "', Паспортные_данные= N'" + maskedTextBox4.Text + "', Пол= N'" + pol +  "' where id = " + id;
                    using (SqlConnection connection = new SqlConnection(connecionString))
                    {
                        connection.Open();
                        SqlDataAdapter adapter = new SqlDataAdapter(sql, connection);
